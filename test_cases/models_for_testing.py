...
def set_all_weights_to_zero(model):
    with torch.no_grad():
        for mod_ in walk(model):
            # conv and linear
            if hasattr(mod_, 'weight'):
                if mod_.weight is not None:
            ...
            if hasattr(mod_, 'running_mean'):
                if mod_.running_mean is not None:
                    mod_.running_mean = torch.zeros(*mod_.running_mean.shape)
            if hasattr(mod_, 'running_var'):
                if mod_.running_var is not None:
